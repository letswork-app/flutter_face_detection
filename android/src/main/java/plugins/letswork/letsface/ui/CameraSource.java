...
public class CameraSource {
    ...
    public CameraSource start() throws IOException, LetsFaceException {
        synchronized (cameraLock) {
            if (camera != null) {
                return this;
            }
    ...
    public CameraSource start(SurfaceHolder surfaceHolder)
            throws IOException, LetsFaceException {
     ...
     */
    @SuppressLint("InlinedApi")
    private Camera createCamera() throws LetsFaceException {
        int requestedCameraId = getIdForRequestedCamera(facing);
        if (requestedCameraId == -1) {
            throw new LetsFaceException("Could not find requested camera.");
        }
        Camera camera = Camera.open(requestedCameraId);

        SizePair sizePair = selectSizePair(camera, requestedPreviewWidth, requestedPreviewHeight);
        if (sizePair == null) {
            throw new LetsFaceException("Could not find suitable preview size.");
        }
        ...
        if (previewFpsRange == null) {
            throw new LetsFaceException("Could not find suitable preview frames per second range.");
        }
