...
class FaceGraphic extends GraphicOverlay.Graphic {
    ...
    public void draw(Canvas canvas) {
        ...
        /*
            ...
            List<Landmark> landmarks = getFace().getLandmarks();

            for (Landmark landmark : landmarks) {
                float cx = scaleX((landmark.getPosition().x + face.getWidth() / 2));
                float cy = scaleY((landmark.getPosition().y + face.getHeight() / 2));
                canvas.drawCircle(cx, cy, 10, boxPaint);
                canvas.drawText(Integer.toString(landmark.getType()), cx, cy, idPaint);
            }
